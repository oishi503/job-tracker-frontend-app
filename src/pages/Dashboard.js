    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ backgroundColor: '#4f46e5', padding: '16px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(79,70,229,0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button onClick={handleLogout} style={{ padding: '7px 16px', backgroundColor: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>
            Logout
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '16px', marginBottom: '32px' }}>
          {[
            ...
            { label: 'Applied', value: stats.applied, color: '#3b82f6', bg: '#eff6ff' },
            { label: 'Interview', value: stats.interview, color: '#f59e0b', bg: '#fffbeb' },
            { label: 'Offer', value: stats.offer, color: '#10b981', bg: '#ecfdf5' },
            { label: 'Rejected', value: stats.rejected, color: '#ef4444', bg: '#fef2f2' },
          ].map(stat => (
            <div key={stat.label} style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderTop: `4px solid ${stat.color}` }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <button onClick={() => setShowForm(!showForm)} style={{ padding: '9px 20px', backgroundColor: '#4f46e5', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600', boxShadow: '0 2px 6px rgba(79,70,229,0.4)' }}>
            + Add Application
          </button>
        </div>
        ...
        {showForm && (
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', marginBottom: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
            <h3 style={{ margin: '0 0 16px', color: '#1e293b' }}>Add New Application</h3>
            <form onSubmit={handleAdd}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                <input style={inputStyle} placeholder="Company Name" value={company} onChange={e => setCompany(e.target.value)} required />
                <input style={inputStyle} placeholder="Role / Position" value={role} onChange={e => setRole(e.target.value)} required />
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button type="button" onClick={() => setShowForm(false)} style={{ padding: '9px 24px', backgroundColor: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
              </div>
            </form>
          </div>
        )}
            ...
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                {['Company', 'Role', 'Date Applied', 'Status', 'Notes', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '14px 16px', textAlign: 'left', fontSize: '13px', fontWeight: '600', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((app, i) => (
                  <tr key={app._id} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: i % 2 === 0 ? 'white' : '#fafafa' }}>
                    <td style={{ padding: '14px 16px', fontWeight: '600', color: '#1e293b' }}>{app.company}</td>
                    <td style={{ padding: '14px 16px', color: '#475569' }}>{app.role}</td>
                    ...
                    <td style={{ padding: '14px 16px', color: '#64748b', fontSize: '13px' }}>{app.notes || '—'}</td>
                    <td style={{ padding: '14px 16px' }}>
...
const inputStyle = {
  ...
  fontSize: '14px', boxSizing: 'border-box', outline: 'none', color: '#1e293b'
};
